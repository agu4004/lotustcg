...
def _locked_merge_inventory_item(user_id: int, card_id: int) -> InventoryItem:
    ...
    # Lock only the inventory_items row. Avoid joinedload here because it emits
    # a LEFT OUTER JOIN to cards; PostgreSQL forbids FOR UPDATE on the nullable
    # side of an outer join. We can lazy-load the card after locking.
    item = (
        InventoryItem.query
